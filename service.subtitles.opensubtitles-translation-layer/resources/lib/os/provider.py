...
class OpenSubtitlesProvider:
    ...
    def __init__(self, api_key, username, password, api_url=None):
       ...
       #     raise ConfigurationError("Username and password must be specified")
        ...
        # Use provided api_url or fall back to default
        self.api_url = api_url if api_url else API_URL
        # Ensure URL ends with /
        if not self.api_url.endswith('/'):
            self.api_url += '/'
        
        logging(f"OpenSubtitlesProvider initialized with API URL: {self.api_url}")
    ...
    def login(self):
        ...
        # build login request
        login_url = self.api_url + API_LOGIN
    ...
    def search_subtitles(self, query: Union[dict, OpenSubtitlesSubtitlesRequest]):
        ...
        try:
            # build query request
            subtitles_url = self.api_url + API_SUBTITLES
...
#           #raise ProviderError("Unable to obtain an authentication token")
    ...
    def download_subtitle(self, query: Union[dict, OpenSubtitlesDownloadRequest]):
        ...
        download_url = self.api_url + API_DOWNLOAD
        download_headers= {}
